...
Migrations live in **one project per provider** — `src/Host/FS.Proxy.Migrations.PostgreSQL` and
`src/Host/FS.Proxy.Migrations.MSSQL` — each foldered **per module/context** (`Catalog/`, `Identity/`, …)
...
dotnet ef migrations has-pending-model-changes --context {X}DbContext \
  --project src/Host/FS.Proxy.Migrations.PostgreSQL --startup-project src/Host/FS.Proxy.Api
...
dotnet run --project src/Host/FS.Proxy.DbMigrator -- list-pending   # to preview first
```
