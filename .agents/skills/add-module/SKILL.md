...
```bash
dotnet sln src/FS.Proxy.slnx add src/Modules/{Name}/Modules.{Name}/Modules.{Name}.csproj
dotnet sln src/FS.Proxy.slnx add src/Modules/{Name}/Modules.{Name}.Contracts/Modules.{Name}.Contracts.csproj
```

Add a `<ProjectReference>` to the runtime module from **both** `FS.Proxy.Api` and `FS.Proxy.DbMigrator`, and reference the runtime project from `FS.Proxy.Migrations.PostgreSQL`.

## Step 5 — Migrations folder

Add a `{Name}/` folder in `src/Host/FS.Proxy.Migrations.PostgreSQL`, then create the initial migration (see **create-migration**) with `--context {Name}DbContext`.

## Step 6 — ⚠️ Register in ALL FOUR places (the footgun)

Identical edits in **both** `FS.Proxy.Api/Program.cs` **and** `FS.Proxy.DbMigrator/Program.cs`:
...
```bash
dotnet build src/FS.Proxy.slnx                  # 0 warnings
