...
|---|---|---|
| Mediator `o.Assemblies` (two markers: Contracts type **and** module type) | `src/Host/FS.Proxy.Api/Program.cs` | Handlers silently undiscovered |
| `moduleAssemblies` array | `src/Host/FS.Proxy.Api/Program.cs` | Module never loaded |
| Mediator assemblies (same pair) | `src/Host/FS.Proxy.DbMigrator/Program.cs` | Migrate/seed misses the module |
| module assemblies array | `src/Host/FS.Proxy.DbMigrator/Program.cs` | Migrate/seed misses the module |
...
- `appsettings.json` (+ `.Development`/`.Production`) live in `src/Host/FS.Proxy.Api/`. DbMigrator links the same files.
