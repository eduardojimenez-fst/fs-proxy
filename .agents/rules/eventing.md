...
The **host** bootstraps eventing once (`FS.Proxy.Api/Program.cs` and `FS.Proxy.DbMigrator/Program.cs`, before `AddModules` so `EventingDbInitializer` migrates the `framework` schema first):
