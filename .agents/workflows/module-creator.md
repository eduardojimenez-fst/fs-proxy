...
You orchestrate a full module bring-up for FS Proxy. **The code recipe lives in the `add-module`
...
5. **⚠️ Register in ALL FOUR places** — Mediator `o.Assemblies` (Contracts marker **and** module type) + `moduleAssemblies` array, in **both** `FS.Proxy.Api/Program.cs` **and** `FS.Proxy.DbMigrator/Program.cs`. Add to `.slnx`; reference the runtime project from Api, DbMigrator, and the Migrations project.
...
dotnet run --project src/Host/FS.Proxy.DbMigrator -- list-pending   # new context shows up
```
