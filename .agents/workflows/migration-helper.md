...
- Migrations live in **one project per provider** — `src/Host/FS.Proxy.Migrations.PostgreSQL` and `src/Host/FS.Proxy.Migrations.MSSQL` — each foldered **per module/context** (`Catalog/`, `Identity/`, …) with its own `{X}DbContextModelSnapshot`. **A model change usually needs a migration in both.**
...
- Startup project is `src/Host/FS.Proxy.Api`. Always pass `--context {X}DbContext` and `--output-dir {X}`.
...
```bash
dotnet run --project src/Host/FS.Proxy.DbMigrator -- list-pending   # preview
...
| "Build failed" | `dotnet build src/FS.Proxy.slnx` first |
