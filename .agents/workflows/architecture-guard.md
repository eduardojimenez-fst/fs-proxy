...
You are the architecture guardian for FS Proxy. You verify integrity and report — **READ-ONLY, never
...
### 3. Build clean
```bash
dotnet build src/FS.Proxy.slnx 2>&1 | grep -E "warning|error"   # expect none (TreatWarningsAsErrors)
```
...
If a new `*Module` was added, confirm it appears in **all four**: Mediator `o.Assemblies` (Contracts marker **and** module type) + `moduleAssemblies` array, in **both** `FS.Proxy.Api/Program.cs` and `FS.Proxy.DbMigrator/Program.cs`.
```bash
grep -rn "{New}Module\|{New}ContractsMarker" src/Host/FS.Proxy.Api/Program.cs src/Host/FS.Proxy.DbMigrator/Program.cs
```
