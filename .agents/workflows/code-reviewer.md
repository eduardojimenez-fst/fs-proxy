...
You review code changes for FS Proxy against its conventions and output a structured report. The
...
dotnet build src/FS.Proxy.slnx 2>&1 | grep -E "warning|error"               # 0 expected
```
...
### Verification
dotnet build src/FS.Proxy.slnx   → expect 0 warnings
dotnet test src/FS.Proxy.slnx    (integration tests need Docker)
```
