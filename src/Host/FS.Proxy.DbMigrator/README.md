...
dotnet run --project src/Host/FS.Proxy.DbMigrator -- apply --tenant root

# Apply only the tenant catalog (no per-tenant pass).
dotnet run --project src/Host/FS.Proxy.DbMigrator -- apply --catalog-only
...
dotnet run --project src/Host/FS.Proxy.DbMigrator -- apply --seed

# Just the seed step (assumes schema is already current).
dotnet run --project src/Host/FS.Proxy.DbMigrator -- seed
...
DOTNET_ENVIRONMENT=Development \
  dotnet run --project src/Host/FS.Proxy.DbMigrator -- seed-demo
```
...
as `FS.Proxy.Api` (both files are linked into the project so they
...
| `DatabaseOptions__MigrationsAssembly`     | Must match the provider — `FS.Proxy.Migrations.PostgreSQL` or `FS.Proxy.Migrations.MSSQL` |
...
```bash
dotnet publish src/Host/FS.Proxy.DbMigrator -c Release \
        ...
        - name: migrator
          ...
          env:
            ...
            - name: DatabaseOptions__MigrationsAssembly
              value: FS.Proxy.Migrations.PostgreSQL   # or FS.Proxy.Migrations.MSSQL
```
...
- name: Migrate database
  ...
  env:
    ...
    DatabaseOptions__Provider: POSTGRESQL                              # or MSSQL
    DatabaseOptions__MigrationsAssembly: FS.Proxy.Migrations.PostgreSQL   # or FS.Proxy.Migrations.MSSQL
```
...
- **Aspire**: `dotnet run --project src/Host/FS.Proxy.AppHost` —
...
- **Raw**: run the migrator once after pulling, before starting the
  ...
  # Then start the API
  dotnet run --project src/Host/FS.Proxy.Api
  ```
