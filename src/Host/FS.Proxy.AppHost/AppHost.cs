...
// Per-app prefix from the AppHost assembly name (FS.Proxy.AppHost -> fsh-starter); namespaces Docker volumes + resource names so multiple FSH apps don't clash.
...
var migrator = builder.AddProject<Projects.FS_Proxy_DbMigrator>($"{appPrefix}-db-migrator")
...
var demoSeeder = builder.AddProject<Projects.FS_Proxy_DbMigrator>($"{appPrefix}-demo-seeder")
    .WithReference(database)
    .WaitFor(database)
...
// API Service
var api = builder.AddProject<Projects.FS_Proxy_Api>($"{appPrefix}-api")
    .WithReference(database)
    .WaitFor(database)
    .WaitFor(redis)
    ...
    .WithEnvironment("MailOptions__DisplayName", "FS Proxy")
