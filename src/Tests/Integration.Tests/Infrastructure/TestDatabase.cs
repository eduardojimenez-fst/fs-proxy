...
public sealed class TestDatabase
{
    ...
    /// <summary>
    /// A well-formed but unreachable connection string for the selected provider.
    /// </summary>
    /// <remarks>
    /// Provisioning-failure tests need a string that passes <c>ConnectionStringValidator</c> and
    /// then fails to connect. The validator parses with the provider's own builder, so a
    /// PostgreSQL-shaped string is rejected outright on SQL Server (`Host=` is not a keyword) and
    /// the tenant is refused with 400 before provisioning is ever attempted.
    /// </remarks>
    public static string UnreachableConnectionString =>
        SelectedProvider == MssqlProvider
            ? "Server=127.0.0.1,1;Database=does_not_exist;User Id=sa;Password=x;Connect Timeout=3;TrustServerCertificate=True"
    ...
    public static TestDatabase Create(string databaseName)
    {
        string migrationsAssembly = SelectedProvider == MssqlProvider
            ? "FS.Proxy.Migrations.MSSQL"
            : "FS.Proxy.Migrations.PostgreSQL";
