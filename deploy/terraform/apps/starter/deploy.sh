#!/usr/bin/env bash
#
# One-command deploy of the FS Proxy Starter Kit to AWS.
#
...
if [[ "$BUILD_API" == true ]]; then
  ...
  dotnet publish "$REPO_ROOT/src/Host/FS.Proxy.Api/FS.Proxy.Api.csproj" \
  ...
  dotnet publish "$REPO_ROOT/src/Host/FS.Proxy.DbMigrator/FS.Proxy.DbMigrator.csproj" \
    -c Release -r linux-x64 \
    /t:PublishContainer \
    -p:ContainerRegistry="$REGISTRY_HOST" \
