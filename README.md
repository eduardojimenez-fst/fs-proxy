...
```bash
dotnet run --project src/Host/FS.Proxy.AppHost
```
...
```bash
dotnet run --project src/Host/FS.Proxy.Api      # needs external Postgres + Redis
```
...
src/
  ...
  Host/
    FS.Proxy.Api/                    API composition root
    FS.Proxy.AppHost/                .NET Aspire orchestrator
    FS.Proxy.DbMigrator/             One-shot migrate / seed runner
    FS.Proxy.Migrations.PostgreSQL/  EF Core migrations
...
- [ ] **Mail** — configure SMTP / SendGrid under `MailOptions` in
      `src/Host/FS.Proxy.Api/appsettings.json`.
...
```bash
dotnet test src/FS.Proxy.slnx       # integration tests require Docker
```
