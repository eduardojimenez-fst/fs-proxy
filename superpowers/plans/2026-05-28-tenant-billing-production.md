...
**Phase A gate:** `dotnet build src/FS.Proxy.slnx` clean; `dotnet test` Billing+Multitenancy+Integration green.
...
- [ ] Migration in `FS.Proxy.Migrations.PostgreSQL` (tenant schema). Full-build before `migrations add`.
- [ ] Commit.
...
## Final
- [ ] `dotnet build src/FS.Proxy.slnx` (TreatWarningsAsErrors) + full `dotnet test` (Docker up).
