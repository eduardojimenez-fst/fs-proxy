...
**Migrations**
- `src/Host/FS.Proxy.Migrations.PostgreSQL/<Billing folder>` — one migration: plan columns +
...
- [ ] **Step 5:** Build. Run: `dotnet build src/FS.Proxy.slnx`. Expected: PASS.
...
**Files:** `src/Host/FS.Proxy.Migrations.PostgreSQL/<Billing folder>/`
...
- [ ] **Step 2:** Add migration (use the repo's EF migration convention / `create-migration` skill —
  ...
  `dotnet ef migrations add BillingPlanIntervalAndInvoicePurpose -c BillingDbContext -p src/Host/FS.Proxy.Migrations.PostgreSQL -s src/Host/FS.Proxy.Api -o <Billing migrations folder>`
...
- [ ] **Step 4:** Apply to a scratch DB: `dotnet run --project src/Host/FS.Proxy.DbMigrator -- apply`.
...
- [ ] **Step 1:** `dotnet build src/FS.Proxy.slnx` (warnings = errors) → PASS.
- [ ] **Step 2:** `dotnet test src/FS.Proxy.slnx` (Docker up for integration) → PASS.
